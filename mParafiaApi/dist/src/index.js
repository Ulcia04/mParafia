...
app.post('/api/admin/login', async (req, res) => {
    ...
    try {
        ...
        if (!isMatch)
            return res.status(401).json({ error: "Błędne dane logowania" });
        const allowedGroupIds = admin.managedGroups.map((ag) => ag.groupId);
...
app.post('/api/upload', verifyAdmin, upload.single('photo'), (req, res) => {
    if (!req.file) {
    ...
    try {
        ...
        const updateData = {
            ...
            managedGroups: {
                ...
                create: (allowedGroupIds || []).map((gId) => ({
...
app.put('/api/groups/:id', verifyAdmin, async (req, res) => {
    const id = req.params.id;
    const { name, description, photoUrl, color } = req.body;
...
app.delete('/api/groups/:id', verifyAdmin, async (req, res) => {
    const id = req.params.id;
...
app.post('/api/events/bulk', verifyAdmin, async (req, res) => {
    try {
...
app.put('/api/announcements/:id/toggle-main', verifyAdmin, async (req, res) => {
    try {
        const id = req.params.id;
...
// -- Q&A --
app.get('/api/questions/all', verifyAdmin, async (req, res) => {
    try {
...
app.post('/api/kancelaria', verifyAdmin, async (req, res) => {
    try {
...
app.put('/api/kancelaria/:id', verifyAdmin, async (req, res) => {
    try {
        const id = req.params.id;
        const { title, photoUrl, content, footer, files } = req.body;
...
app.delete('/api/kancelaria/:id', verifyAdmin, async (req, res) => {
    try {
        const id = req.params.id;
