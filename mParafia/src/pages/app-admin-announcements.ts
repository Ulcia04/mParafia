...
interface Announcement { id: number; title: string; content: string; date: string; isMain: boolean; }
...
export class AppAdminAnnouncements extends LitElement {
  ...
  async firstUpdated() { await this.fetchAnnouncements(); }
  ...
  async handleSubmit(e: Event) {
    ...
    if (!title || !date || !content) return alert('Wypełnij wszystkie pola!');

    this.isSubmitting = true;
    try {
      const endpoint = this.editingId ? `/announcements/${this.editingId}` : '/announcements';
      ...
      if (response.ok) { this.resetForm(); await this.fetchAnnouncements(); }
      else { alert('Błąd zapisu ogłoszenia.'); }
  ...
  async toggleMain(id: number) {
    try {
      const response = await apiFetch(`/announcements/${id}/toggle-main`, { method: 'PUT' });
      if (response.ok) await this.fetchAnnouncements();
    } catch (error) { console.error('Błąd:', error); }
  }
  ...
  startEditing(ann: Announcement) {
    ...
    this.dateInput.value = new Date(ann.date).toISOString().split('T')[0];
    this.contentInput.value = ann.content;
    window.scrollTo({ top: 0, behavior: 'smooth' });
  }

  resetForm() { this.editingId = null; this.titleInput.value = ''; this.dateInput.value = ''; this.contentInput.value = ''; }
  private formatDate(dateStr: string) { return new Date(dateStr).toLocaleDateString('pl-PL'); }
  ...
  render() {
    return html`
      ...
      <div class="list-section">
        ...
        ${this.announcements.length === 0 ? html`<p style="color: var(--color-wood-dark);">Brak ogłoszeń w bazie.</p>` : this.announcements.map(ann => html`
            <div class="announcement-item ${ann.isMain ? 'is-main' : ''}">
              <sl-icon class="main-tick ${ann.isMain ? 'is-active' : ''}" name="${ann.isMain ? 'check-circle-fill' : 'check-circle'}" @click="${() => this.toggleMain(ann.id)}" title="${ann.isMain ? 'Odznacz jako główne' : 'Ustaw jako główne'}"></sl-icon>
              <div class="announcement-header"><div class="announcement-info"><h4>${ann.title} ${ann.isMain ? html`<sl-badge variant="success" pill>Strona Główna</sl-badge>` : ''}</h4><span>Data: ${this.formatDate(ann.date)}</span></div></div>
              <div class="actions">
                <sl-button size="small" class="btn-edit" @click="${() => this.startEditing(ann)}"><sl-icon slot="prefix" name="pencil"></sl-icon>Edytuj</sl-button>
                <sl-button size="small" class="btn-delete" @click="${() => this.handleDelete(ann.id)}"><sl-icon slot="prefix" name="trash"></sl-icon>Usuń</sl-button>
              </div>
            </div>
          `)}
      </div>
